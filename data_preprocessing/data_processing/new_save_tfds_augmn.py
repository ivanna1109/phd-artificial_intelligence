...
def load_all_data_into_maps(
    ...
    # Prolazimo kroz sve stavke u parent direktorijumu MOL fajlova
        ...
        if os.path.isdir(subfolder_path):
            ...
            for mol_file_name in os.listdir(subfolder_path):
                if mol_file_name.lower().endswith('.mol'):
                    ...
                    # ### STANDARDIZACIJA NA IME MOL FAJLA ###
...
def process_and_combine_molecular_data(
    ...
    for csv_name, df in csv_dataframes.items():
        ...
        # Zatim standardizacija na celu kolonu
        ...
        for mol_name, row_data in df_indexed.iterrows():
            if mol_name in current_mol_paths_map:
                ...
                label_value = float(label_value) # da je float

                # Pribavljanje globalnih karakteristika
                ...
                if mol_name in all_molecules_processed_data:
                    ...
                    total_skipped_molecules += 1 #  duplikati preskočenim
...
# --- Globalne definicije ( UNIFICIRANI nazivi koje želimo u izlazu) ---
...
# --- POMOĆNE FUNKCIJE ZA EKSTRAKCIJU GRAFOVSKIH FEATURE-A ---
...
def process_mols_for_gnn_dataset(all_molecules_processed_data: dict) -> list[dict]:
    ...
    for i, (mol_name, data) in enumerate(all_molecules_processed_data.items()):
        ...
        try:
            ...
            if not node_features: # Molekul bez atoma? Vrlo retko, ali proveriti.
            ...
            # Edge Features and Adjacency Matrix
            edge_features = []
            adjacency_indices = [] 
            adjacency_values = [] 
            ...
            # Ako nema veza (npr. monoatomski molekul), prazni array-e sa ispravnim dimenzijama
...
# This function will focus on converting NumPy arrays (which may be augmented) to TF Tensors.
...
def add_graph_features_to_mols(mol_dict):
    """Učitava MOL fajl i dodaje graf features originalnim molekulima."""
    processed_dict = {}
    for mol_name, data in mol_dict.items():
        try:
            # Koristimo originalni put do MOL fajla iz metapodataka
            base_graph_features = _extract_base_graph_features_from_mol_file(data['mol_file_path'], mol_name)
            processed_dict[mol_name] = {**data, **base_graph_features}
        except Exception as e:
            print(f"Upozorenje: Preskače se molekul '{mol_name}' zbog greške pri ekstrakciji feature-a: {e}")
            continue
...
def create_tf_datasets_from_predefined_ids(
    all_molecules_processed_data: dict,
    train_ids: list[str],
    val_ids: list[str],
    test_ids: list[str],
    ...
    # mol_ids i labels više nisu potrebni za podelu, ali su potrebni za određivanje dimenzija
    mol_ids = list(all_molecules_processed_data.keys()) 

    print(f"\nUkupno podataka (Original + Augmentovano): {len(mol_ids)}")
    print(f"Dataset veličine: Train ({len(train_ids)} uzoraka), Val ({len(val_ids)} uzoraka), Test ({len(test_ids)} uzoraka).")

    # Određivanje oblika i tipova za tf.data.Dataset (output_signature)
    try:
        dummy_atom_dim = get_atom_features(Chem.Atom(0)).shape[0]
    except Exception:
        # Prikaz warninga se može ostaviti ili ukloniti
        dummy_atom_dim = 12
...
if __name__ == "__main__":
    test_base_dir = '/home/ivana-milutinovic/Documents/Doktorske/Prva godina/Vestacka inteligencija/git_repo/phd-artificial_intelligence/data_preprocessing/data_processing/data'
    ...
    # 1. Procesuiranje i kombinovanje molekularnih metapodataka (BEZ graf feature-a)
    ...
    # 2. PODELA ORIGINALNIH MOLEKULA (ENTITETA)
    
    original_mol_ids = list(all_molecules_metadata.keys())
    original_labels = np.array([all_molecules_metadata[mol_id]['label'] for mol_id in original_mol_ids])

    # Podešavanje split ratio-a (koristimo 0.2 i 0.25 kao što ste naveli, za originalne podatke)
    TEST_SPLIT = 0.2
    VAL_RATIO_OF_TRAIN_VAL = 0.25
    RANDOM_SEED = 42

    # Podela na Train+Val i Test
    train_val_ids_orig, test_ids_orig, train_val_labels_orig, _ = train_test_split(
        original_mol_ids, original_labels,
        test_size=TEST_SPLIT,
        stratify=original_labels,
        random_state=RANDOM_SEED
    )

    # Podela Train+Val na Train i Val
    train_ids_orig, val_ids_orig, _, _ = train_test_split(
        train_val_ids_orig, train_val_labels_orig,
        test_size=VAL_RATIO_OF_TRAIN_VAL,
        stratify=train_val_labels_orig,
        random_state=RANDOM_SEED
    )

    # Kreiranje podskupova metapodataka
    train_mols_orig = {mid: all_molecules_metadata[mid] for mid in train_ids_orig}
    val_mols_orig = {mid: all_molecules_metadata[mid] for mid in val_ids_orig}
    test_mols_orig = {mid: all_molecules_metadata[mid] for mid in test_ids_orig}

    print(f"\n--- Podela Originalnih Molekula (PRE Augmentacije) ---")
    print(f"Original Train ID-jeva: {len(train_ids_orig)}")
    print(f"Original Validation ID-jeva: {len(val_ids_orig)}")
    print(f"Original Test ID-jeva: {len(test_ids_orig)}")
    
    # 3. AUGMENTACIJA SAMO TRENING SKUPA
    # augment_molecular_data istovremeno dodaje graf features i stvara augmentovane kopije
    print("\n--- Pokretanje Augmentacije na TRENING SKUPU ---")
    augmented_train_mols = augment_molecular_data(
        train_mols_orig,
        augment_label_1=True,
        num_augmentations_per_sample=3 #broj svakog molekula koji ce se augmentovati u trening skupu
    )
    print(f"Train skup nakon augmentacije (Original + Aug): {len(augmented_train_mols)}")

    # 4. DODAVANJE GRAF FEATURE-A VAL i TEST SKUPOVIMA (BEZ AUGMENTACIJE)
    print("\n--- Dodavanje graf feature-a za Val i Test skupove (bez augmentacije) ---")
    final_val_mols = add_graph_features_to_mols(val_mols_orig)
    final_test_mols = add_graph_features_to_mols(test_mols_orig)

    # 5. KREIRANJE KONAČNIH LISTA ID-jeva i JEDINSTVENOG REČNIKA
    
    # Finalni ID-jevi (uključujući augmentovane ID-jeve u trening skupu)
    train_ids_final = list(augmented_train_mols.keys())
    val_ids_final = list(final_val_mols.keys())
    test_ids_final = list(final_test_mols.keys())

    # Spajanje svih podataka u jedan rečnik koji se prosleđuje funkciji za kreiranje dataseta
    final_all_data = {**augmented_train_mols, **final_val_mols, **final_test_mols}
    
    print(f"\nTotalni finalni uzorci za obradu (Train+Val+Test): {len(final_all_data)}")


    # 6. KREIRANJE TF DATASETA koristeći unapred podeljene ID-jeve
    # Ovde koristimo preimenovanu funkciju 'create_tf_datasets_from_predefined_ids'
    train_ds, val_ds, test_ds = create_tf_datasets_from_predefined_ids(
        final_all_data,
        train_ids_final,
        val_ids_final,
        test_ids_final,
    ...
    # 7. Čuvanje kreiranih dataseta
    output_dir = '/home/ivana-milutinovic/Documents/Doktorske/Prva godina/Vestacka inteligencija/git_repo/phd-artificial_intelligence/data_preprocessing/data/tfrecords_augmented_new'
