...
def process_and_combine_molecular_data(
    ...
    column_mapping: dict, # Koristimo ACTUAL_COLUMN_NAMES_PER_ASSAY_TYPE
    ...
    for csv_name, df in csv_dataframes.items():
        ...
        # standardizaciju na celu kolonu
        ...
        # Postavljamo 'mol_id_col_source' kao indeks DataFrame-a
        ...
        for mol_name, row_data in df_indexed.iterrows():
            if mol_name in current_mol_paths_map:
                ...
                # Provera postojanja putanje (iako bi mapa već trebala da ima samo postojeće putanje)
                ...
                label_value = float(label_value) # Potvrda da je float
                ...
                if mol_name in all_molecules_processed_data:
                    ...
                    total_skipped_molecules += 1 # duplikat preskočen
                    #continue
...
def process_mols_for_gnn_dataset(all_molecules_processed_data: dict) -> list[dict]:
    ...
    for i, (mol_name, data) in enumerate(all_molecules_processed_data.items()):
        ...
        try:
            ...
            if not node_features: # Molekul bez atoma? Vrlo retko, ali proveriti ipak.
            ...
            processed_data = {
                ...
                'esr': data.get('esr', 'unknown'), # Sa .get() je sigurnije da ćemo imati vrednost
                'label': data['label'],
...
if __name__ == "__main__":
    ...
    # Cuvanje kreiranin datasetova
    # Direktorijum 'tf_datasets' ako ne postoji
