...
from training.models.gSage import GraphSageModel
import pandas as pd
...
batch_size = 60 #po optuna proracunu
...
num_epoch = 50
print("Tri skupa prilagodjena za trening, val i test kreirani da bi mogli pustiti u GSage.")
hidden_units = 99  # Skriveni slojevi gSage
dense_units = [93, 241]
dropout_rate = 0.21

model = GraphSageModel(num_features, num_labels, hidden_units, dense_units, dropout_rate)
print("Kreiran gSAGE model.")
...
with open(f'training/{final_dir}/gSage/model_summary-e{num_epoch}.txt', 'w') as f:
...
history_df.to_csv(f'training/{final_dir}/gSage/gSage_model-e{num_epoch}.csv', index=False)

print("Cuvanje tezina modela..")
model.save_weights(f'training/{final_dir}/gSage/gSage_model-e{num_epoch}.h5')

output_file_path = f"training/{final_dir}/gSage/gSage_test_results-e{num_epoch}.txt"
