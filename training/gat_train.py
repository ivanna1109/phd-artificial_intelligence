...
from training.models.gat import GATModel
import pandas as pd
...
with open(f'training/{final_dir}/gat/model_summary-gat-e{num_epoch}.txt', 'w') as f:
    model.summary(print_fn=lambda x: f.write(x + '\n'))
print("Zavrseno pisanje u fajl.")

print("Pisemo history treninga..")
history_df = pd.DataFrame(history.history)
history_df.to_csv(f'training/{final_dir}/gat/gat_history_e{num_epoch}.csv', index=False)

print("Cuvanje tezina modela..")
model.save_weights(f'training/{final_dir}/gat/gat_model_e{num_epoch}.h5')

output_file_path = f"training/{final_dir}/gat/gat_test_results-e{num_epoch}.txt"
