...
import data_preprocessing.tfds_load_multi as tfds
import training.metrics.calculate_metrics as metric
from training.models.gcn import GCN
import spektral_data_multi.tf_to_spektral as tf_to_s
from spektral_data_multi.spektral_dataset import MyDataset
import pandas as pd
...
dataset_dir = '/home/ivanam/BIO-Info/bio_info//data_preprocessing/data/tfrecords_multi_b_oversampled'
print("GCN Training..............................................")
...
num_labels = 5     # Broj klasa (podela po jačini aktivnosti u odnosu ac50)
hidden_units = 218  # Skriveni slojevi GCN sa 64 jedinice
...
final_dir = 'multiclass_results/without_optuna'
with open(f'training/{final_dir}/gcn_oversampled/model_summary-gcn-max-e{num_epoch}.txt', 'w') as f:
    model.summary(print_fn=lambda x: f.write(x + '\n'))
print("Zavrseno pisanje u fajl.")

print("Pisemo history treninga..")
history_df = pd.DataFrame(history.history)
history_df.to_csv(f'training/{final_dir}/gcn_oversampled/gcn_history-max-e{num_epoch}.csv', index=False)

print("Cuvanje tezina modela..")
model.save_weights(f'training/{final_dir}/gcn_oversampled/gcn_model-max-e{num_epoch}.h5')

output_file_path = f"training/{final_dir}/gcn_oversampled/gcn_test_results-max-e{num_epoch}.txt"
