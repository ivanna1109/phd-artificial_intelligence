...
from training.models.gin import GINModel
import pandas as pd
...
batch_size = 31 #po optuna proracunu
...
hidden_units = 232  # Skriveni slojevi gin
dense_units = [130, 148]
dropout_rate = 0.13
...
with open(f'training/{final_dir}/gin/model_summary-gin_e{num_epoch}.txt', 'w') as f:
    model.summary(print_fn=lambda x: f.write(x + '\n'))
print("Zavrseno pisanje u fajl.")

print("Pisemo history treninga..")
history_df = pd.DataFrame(history.history)
history_df.to_csv(f'training/{final_dir}/gin/gin_history-e{num_epoch}.csv', index=False)

print("Cuvanje tezina modela..")
model.save_weights(f'training/{final_dir}/gin/gin_model-e{num_epoch}.h5')

output_file_path = f"training/{final_dir}/gin/gin_test_results-e{num_epoch}.txt"
